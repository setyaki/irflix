...
export default function Home() {
    ...
    useEffect(() => {
        fetch('/api/getMovieList').then((result) => result.json()).then((values) => {
            
            setData(values)
            
            fetch(`/api/getMovieDetails/${values[0].id}`).then((result1) => result1.json()).then((value1) => {  
                setMovieDetail(value1)
            })

            fetch(`api/getMovieVideo/${values[0].id}`).then((result2) => result2.json()).then((value2) => {
                setMovieVideo(value2)
            }) 
            // console.log("Video Data:", movieVideo)
        })
    },[])
    ...
    <nav className="invisible sm:visible bg-gradient-to-b from-slate-950 to-slate-950/0 text-white px-9 pb-9 pt-6 w-full h-auto fixed top-0 left-0 right-0 z-50">
        ...
        {!movieDetail ? <div>loading</div> : <Hero data={movieDetail} video={movieVideo}/>}
